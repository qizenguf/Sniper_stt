...
int Cache::lowestDiffLine(IntPtr addr, Byte* fill_buff,
      ...
      SubsecondTime now, CacheCntlr *cntlr, int shiftSize, int flipSize)
      {
			IntPtr tag;
			UInt32 set_index;
			splitAddress(addr, tag, set_index);
			...
			for(unsigned int i =0 ; i < m_associativity; i++)
			{
				...
				int t = lineCompare(read_buff, evict_buff, 64, shiftSize, flipSize);
				if(t < lowestDiffBits) lowestDiffBits = t;
			}
...
int Cache::lineCompare( Byte* ablock, Byte* bblock, int size, int shiftSize, int flipSize){
	...
	for (int i =0; i<size; i += shiftSize){
		...
		if(totalbits < minimal_bits) minimal_bits=totalbits;		
	}
	return minimal_bits;	
}
