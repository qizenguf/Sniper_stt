...
#  define AGID() fprintf(stderr, "%2u ", Sim()->getCoreManager()->getCurrentCoreID());
//#  define MYLOG(...) LOCKED(LOGID(); fprintf(stderr, __VA_ARGS__); fprintf(stderr, "\n");)
#  define MYLOG(...) {}
#  define AOLOG(...)  {}
//#  define AOLOG(...)  fprintf(stderr, __VA_ARGS__);
   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   m_shiftSize = Sim()->getCfg()->getInt("perf_model/STT/shift_size");
   m_flipSize =  Sim()->getCfg()->getInt("perf_model/STT/flip_size");
   //m_sector =  Sim()->getCfg()->getInt("perf_model/STT/sector_size");
   ...
   if (isMasterCache())
   {
      ...
      m_master->m_cache = new Cache(name,
            ...
            Sim()->getCfg()->getInt("perf_model/"+cache_params.configName+"/sector_size"),
   ...
   if (is_last_level_cache)
   {
	  ...
	  evicted_bits = 0;
	  optimal_bits = 0;
	  evicted_is_optimal = 0;
	  eviction_times = 0;
	  evicted_nearby_bits = 0;
	  evicted_nearby = 0;
	  for(int i =0 ; i<513; i++){
      ...
      registerStatsMetric(namez, core_id, "evicted_bits", &evicted_bits);
      registerStatsMetric(namez, core_id, "optimal_bits", &optimal_bits);
      registerStatsMetric(namez, core_id, "evicted_is_optimal", &evicted_is_optimal);
	  registerStatsMetric(namez, core_id, "eviction_times", &eviction_times);
	  registerStatsMetric(namez, core_id, "evicted_nearby_times", &evicted_nearby);
	  registerStatsMetric(namez, core_id, "evicted_nearby_bits", &evicted_nearby_bits);
	  for(int i =0 ; i<513; i++){
		  String numi = itostr(i);
		  if(i<100) numi = "0"+ numi;
		  if(i<10) numi = "0" + numi;
		  String res =  "evicted_bits_cnt-" + numi; 
		  
		  registerStatsMetric(namez, core_id, res, &evicted_bits_cnt[i]);

		  res = "optimal_bits_cnt-" + numi;
		  registerStatsMetric(namez, core_id, res, &optimal_bits_cnt[i]);
	  }
   }
}
...
MYLOG("processMemOpFromCore l%d next hit = %d", m_mem_component, next_cache_hit);
	if( hit_where != HitWhere::where_t(MemComponent::L2_CACHE) )  {
		//long ins = 0;
		/*core_id_t n = (core_id_t)Sim()->getConfig()->getTotalCores();
		for(core_id_t core_id = 0; core_id < n; ++core_id)
      			ins +=  (long)Sim()->getCoreManager()->getCoreFromID(core_id)->getPerformanceModel()->getInstructionCount();*/
		//SubsecondTime t_wow = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
		//AOLOG("0x%lx %c %ld %d\n", ca_address, 'R', t_wow.getNS(), (int)Sim()->getCoreManager()->getCurrentCoreID());
		SubsecondTime t_wow = getShmemPerfModel()->getElapsedTime(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD);
		//maxCycle = t_wow.getNS() > maxCycle ? t_wow.getNS(): maxCycle;
		char tp = 'R';
		if (mem_op_type == Core::WRITE) tp = 'W';
		AOLOG("0x%lx %c %ld %d %lx\n", ca_address, tp, t_wow.getNS(), (int)Sim()->getCoreManager()->getCurrentCoreID(), eip );
	} //add by aoge
      if (next_cache_hit) {
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type,  IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
  ...
  /* if( m_mem_component == MemComponent::L3_CACHE) {
	...
	core_id_t n = (core_id_t)Sim()->getConfig()->getTotalCores();
		for(core_id_t core_id = 0; core_id < n; ++core_id)
      		ins +=  (long)Sim()->getCoreManager()->getCoreFromID(core_id)->getPerformanceModel()->getInstructionCount();
		SubsecondTime t_wow = Sim()->getCoreManager()->getCoreFromID((core_id_t)0)->getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
		AOLOG("0x%lx %c %ld %d\n", address, '3', t_wow.getNS(), (int)Sim()->getCoreManager()->getCurrentCoreID() );
   }*/
...
MYLOG("evicting @%lx", evict_address);
      ...
      )
      {
		...
		if (!m_next_cache_cntlr) {
		 eviction_times ++;
		 int optimal_t = m_master->m_cache->lowestDiffLine(address, data_buf,
				&eviction, &evict_address, &evict_block_info, evict_buf,
				getShmemPerfModel()->getElapsedTime(thread_num), this, m_shiftSize, m_flipSize);
         IntPtr tag, tag2;
         UInt32 set_index, set_index2;
         m_master->m_cache->splitAddress(address,tag, set_index);
         m_master->m_cache->splitAddress(evict_address,tag2, set_index2);
         assert(set_index == set_index2);
         
         int evicted_t = m_master->m_cache->lineCompare(data_buf, evict_buf, getCacheBlockSize(), m_shiftSize, m_flipSize);
         if(tag/64 == tag2/64) {
			evicted_nearby++;
   ...
   else
   {
      ...
      if (cache_block_info->getCState() == CacheState::MODIFIED) {
		...
		/*if( m_mem_component == MemComponent::L2_CACHE) {
			...
			for(core_id_t core_id = 0; core_id < n; ++core_id)
      				ins +=  (long)Sim()->getCoreManager()->getCoreFromID(core_id)->getPerformanceModel()->getInstructionCount();
			AOLOG("0x%lx %c %ld", address, 'W', ins);
		}*/
...
MYLOG(" ");
   ...
   if( m_mem_component == MemComponent::L3_CACHE) {
	...
	/*core_id_t n = (core_id_t)Sim()->getConfig()->getTotalCores();
		...
		SubsecondTime t_wow = getShmemPerfModel()->getElapsedTime(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD);
		//maxCycle = t_wow.getNS() > maxCycle ? t_wow.getNS(): maxCycle;
		AOLOG("0x%lx %c %ld %d\n", address, 'B', t_wow.getNS(), (int)Sim()->getCoreManager()->getCurrentCoreID());
   }
   if (m_master->m_evicting_buf && (address == m_master->m_evicting_address)) {
